...
class BaseCorrector(object):
    def __init__(self):
        self.data = open(os.path.abspath(os.path.join(
            DATA_PATH, 'big.txt')), mode='r').read()
...
class SpellCorrector(BaseCorrector):
    ...
    def correct_string(self, string_corpus):
        ...
        for w in words:
            ...
            if corrected_word.lower() != real_word.lower():
                mistakes += 1
                new_word = "<spelling data-old='{0}'>".format(
                    w) + w.replace(real_word, corrected_word) + "</spelling>"
    ...
    def correct(self, word):
        w = word.lower()
        candidates = self.known([w]) or self.known(
            self.edits1(w)) or self.known_edits2(w) or [w]
